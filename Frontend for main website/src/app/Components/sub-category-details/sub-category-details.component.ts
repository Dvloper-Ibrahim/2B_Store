// SubCategoryDetailsComponent.ts
...
selectedPage: number = 1;
  constructor(
    ...
    private prodService: ProductsPagesService,
    private sortProduct: SortProductService
  ) {}

  ngOnInit(): void {
    this.subCategoryID = Number(this.route.snapshot.paramMap.get('subCategoryID'));
    this.subCatService.getSubCategoryByID(this.subCategoryID).subscribe(data => {
      data.name = this.localLang == 'ar' ? data.nameAR : data.nameEN;
      this.subCategory = data;
    });

    this.isParent = this.subCategory.subCategoryId == null;

    this.subCatService.getSubCategoriesInSubCategory(this.subCategoryID).subscribe(data => {
      data.forEach(item => {
    ...
    if (this.isParent) {
      this.prodService.getProductsBySubCategoryId(this.subCategoryID).subscribe(data => {
        data.forEach(prod => {
